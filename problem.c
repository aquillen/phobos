...
// for volume to be the same, adjusting here!!!!
   ...
   printf("spin_period=%.6f\n",speriod);
   fprintf(fpr,"spin_period %.6f\n",speriod);
   ...
   printf("Young's_modulus=%.6f\n",Emush);
   printf("Young's_modulus_big=%.6f\n",Emush_big);
   fprintf(fpr,"Young's_modulus %.6f\n",Emush);
   fprintf(fpr,"Young's_modulus_big %.6f\n",Emush_big);
   ...
   if (npointmass >0){ 
      ...
      for(int ipp = 1;ipp<npointmass;ipp++){
          ...
          fprintf(fpr,"pointm%d_mm %.4f\n",ipp,omp);
          fprintf(fpr,"pointm%d_period %.2f\n",ipp,2.0*M_PI/omp);
